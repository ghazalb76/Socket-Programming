...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        // Fork a process after accepting new clinet to let the parent process continue listening for new clients
        int f = fork();
        // If it's the process of child execute these lines else continue listening
        if (f==0){
            memset(buf, 0, 2047);
